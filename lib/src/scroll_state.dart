...
class ScrollState with ChangeNotifier {
  ...
  static double calcMaxDelta(ScrollController controller, double delta) {
    return delta > 0
        ? math.min(controller.position.pixels + delta, controller.position.maxScrollExtent) -
            controller.position.pixels
        : math.max(controller.position.pixels + delta, controller.position.minScrollExtent) -
            controller.position.pixels;
  }

  void handleDesktopScroll(PointerSignalEvent event, double scrollSpeed, Curve animationCurve,
        ...
        handlePipelinedScroll = () {
          ...
          if (!outOfBounds && shouldLock) {
            ...
            if (lastLock != null || outOfBounds)
              controller.jumpTo(
                  lastLock != null ? posPixels : (currPos - calcMaxDelta(controller, currDelta)));
            lastLock = null;
